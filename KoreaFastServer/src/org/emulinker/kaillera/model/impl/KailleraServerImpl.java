...
public class KailleraServerImpl implements KailleraServer, Executable
{
	...
	public synchronized void login(KailleraUser user) throws PingTimeException, ClientAddressException, ConnectionTypeException, UserNameException, LoginException
	{
		...
		if (access < AccessManager.ACCESS_ADMIN)
			...
			announce(EmuLang.getString("KailleraServerImpl.UserWelcomeMessage", user.getName(), (user.getConnectSocketAddress().getAddress().getHostAddress()).substring(0,9)+"..."), false, null);
		///
		
		/*
		 * try { Thread.sleep(20); } catch(Exception e) {} userImpl.addEvent(new
		 * InfoMessageEvent(user, getReleaseInfo().getProductName() + " v" +
		 * getReleaseInfo().getVersionString() + ": " +
		 * getReleaseInfo().getReleaseDate() +
		 * " - https://koreafastserver.wixsite.com/kaillera"));
		 */
	...
	public synchronized void quit(KailleraUser user, String message) throws QuitException, DropGameException, QuitGameException, CloseGameException
	{
		...
		if (access < AccessManager.ACCESS_SUPERADMIN && user.getServer().getAccessManager().isSilenced(user.getSocketAddress().getAddress())){
			quitMsg = "https://koreafastserver.wixsite.com/kaillera";
		}
	...
	public synchronized KailleraGame createGame(KailleraUser user, String romName) throws CreateGameException, FloodException
	{
		...
		///
		announce(EmuLang.getString("KailleraServerImpl.UserCreatedGameAnnouncement", user.getName(), game.getRomName(), (user.getConnectSocketAddress().getAddress().getHostAddress()).substring(0,9)+"..."), false, null);
		///
