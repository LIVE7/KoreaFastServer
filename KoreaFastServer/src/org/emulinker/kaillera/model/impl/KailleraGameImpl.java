...
public final class KailleraGameImpl implements KailleraGame
{
	...
	public synchronized int join(KailleraUser user) throws JoinGameException
	{
		...
		//{
			...
			announce("Korea Fast Server. (KFS)", user);
			announce(EmuLang.getString("KailleraGameImpl.Help"), user);
			announce("************************", user);
...
//			announce("************************", user);
			...
			if(autoFireDetector != null)
			{
				...
				{
					announce(EmuLang.getString("KailleraGameImpl.AutofireDetectionOn"), user); //$NON-NLS-1$
					announce(EmuLang.getString("KailleraGameImpl.AutofireCurrentSensitivity", autoFireDetector.getSensitivity()), user); //$NON-NLS-1$
				}
				else
				{
					announce(EmuLang.getString("KailleraGameImpl.AutofireDetectionOff"), user); //$NON-NLS-1$
				}
