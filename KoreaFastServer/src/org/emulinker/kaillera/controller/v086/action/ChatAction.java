...
public class ChatAction implements V086Action, V086ServerEventHandler
{
	...
	private void checkCommands(V086Message message, V086Controller.V086ClientHandler clientHandler) throws FatalActionException{
		...
		if(userN.getAccess() >= AccessManager.ACCESS_ADMIN){
			try{
			...
			if(doCommand){
				...
				if(((Chat) message).getMessage().equals("/alivecheck")){
					try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", ":ALIVECHECK=Korea Fast Server Alive Check: You are still logged in."));} catch(Exception e) {}
				}
					...
					try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", "VERSION: " + releaseInfo.getProductName() + " " + releaseInfo.getVersionString() + " release: " + releaseInfo.getReleaseDate()));} catch(Exception e) {}
				}
					...
					if(clientHandler.getUser().getAccess() >= AccessManager.ACCESS_ADMIN){
						try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", EmuLang.getString("ChatAction.me"))); } catch(Exception e) {}
						...
						try { Thread.sleep(20); } catch(Exception e) {}
						try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", EmuLang.getString("ChatAction.ignore"))); } catch(Exception e) {}
						...
						try { Thread.sleep(20); } catch(Exception e) {}
						try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", EmuLang.getString("ChatAction.msg"))); } catch(Exception e) {}
						...
						try { Thread.sleep(20); } catch(Exception e) {}
						try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", EmuLang.getString("ChatAction.myip"))); } catch(Exception e) {}
						...
						try { Thread.sleep(20); } catch(Exception e) {}		
						try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", EmuLang.getString("ChatAction.version"))); } catch(Exception e) {}
						...
						try { Thread.sleep(20); } catch(Exception e) {}	
						try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", EmuLang.getString("ChatAction.finduser"))); } catch(Exception e) {}
					...
					for (KailleraUserImpl user : clientHandler.getUser().getUsers())
					{
						...
						if (user.getName().toLowerCase().contains(str.toLowerCase()))
						{
							...
							///
							//sb.append(", Ip: (");
							//sb.append((user.getConnectSocketAddress().getAddress().getHostAddress()).substring(0,9)+"...");
							//sb.append(")");
							///
							sb.append(", Access: ");
